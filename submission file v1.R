...
VERSION="R version 3.5.2 (2019-03-11)"
...
missing_001 <- function(data){
  ...
  select(
    comp_week, comp_wend,
    comp_bed,
    sex, weight_16, height_16, iq, bmi_16,
    child_bull,
    exercise, creat_14,
    play_wend, play_week,
    has_dep_diag,
  ...
  MIobj <- mice::mice(MIvars, m=10, maxit=10, pred=quickpred(MIvars))

  #create single tall data frame with all imputed datasets binded lengthways
  MIdatalong <- complete(MIobj, action='long',include=TRUE)

  MIdatalong
}

transformations_002 <- function(data){
  library('tidyverse')
  library('mice')

  # modify variables after imputation
  newdata <- data %>%
    mutate(
...
specify_model <- function(data){
  ...
  mi.fits <- with(
              ...
              glm(
                  formula =
                    depression ~ comp_comb + dep_band_15_red + comp_bed + creat_14 + play_comb + child_bull + sex ,
                  family = binomial(),
                )
              )
...
data3 <- missing_001(data2)
data4 <- transformations_002(data3)
data5 <- depression_001(data4)
data6 <- computer_use_001(data5)
data7 <- additional_001(data6)
results <- specify_model(data7)
